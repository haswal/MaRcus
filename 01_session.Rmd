...
1. What does the `group =` aesthetic do? Run the code below and explain what you see. 
    ...
    What does `se = FALSE` in `geom_smooth` do? What happens if you set if to TRUE in this case?
...
1. Do your best to recreate the plot below. Check out [this link](https://www.datanovia.com/en/blog/how-to-plot-a-smooth-line-using-ggplot2/) to figure out how to fit a straight line to the data (instead of the wiggly trajectories we have seen this geom produce so far).
...
1. Assign the "correct" colors to the `Phase (color)` variable in the plot above (meaning that the dots in the scatter plot representing for example `1-lightgray` should have the color `"lightgray"` and so forth). Take a look at [this link](https://www.statology.org/color-by-factor-ggplot2/) (Example 2) to figure out how to manually change the default color scheme.

    Run `?scale_color_manual` in the R console (this will only work if the `ggplot2` package has been loaded). What happens? Find the paper talking about color vision deficiencies (and read it if you are interested). There is a color scheme in the `ggthemes` package made speficically with color vision deficiencies in mind. Can you find it in the `ggthemes` documentation?
    
    What does the `c()` function do?
    
1. After changing the colors in the exercise above, the color guide (the list of colors to the right of plot panel, sometimes also called the color legend) is a bit redundant. Can you figure out how to remove it? Google will be your friend here, and good search terms could be *ggplot2*, *color guide* and *remove*. 
